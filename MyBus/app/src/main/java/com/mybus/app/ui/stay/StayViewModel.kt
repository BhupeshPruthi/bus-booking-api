...
class StayViewModel @Inject constructor(
    ...
    fun updateCouponInput(value: String) {
        val normalized = value.uppercase().filter { it.isLetterOrDigit() || it == '_' || it == '-' }
            .take(50)
        val quote = _uiState.value.quote
        val appliedCode = quote?.couponCode
        _uiState.value = _uiState.value.copy(
            couponInput = normalized,
            quote = if (appliedCode != null && appliedCode != normalized) {
                quote.copy(
                    discountAmount = 0.0,
                    couponCode = null,
                    totalAmount = quote.subtotalAmount
                )
            } else {
                quote
            },
    ...
    fun applyCoupon() {
        val state = _uiState.value
        val code = state.couponInput.trim()
        val currentQuote = state.quote
        if (currentQuote == null || !currentQuote.canFulfill) {
            _uiState.value = state.copy(couponError = "Check availability before applying a coupon.")
            return
        }
        if (code.isBlank()) {
            _uiState.value = state.copy(
                quote = currentQuote.copy(
                    discountAmount = 0.0,
                    couponCode = null,
                    totalAmount = currentQuote.subtotalAmount
                ),
                couponError = null,
                couponMessage = null
            )
        ...
        viewModelScope.launch {
            _uiState.value = state.copy(isLoading = true, couponError = null, couponMessage = null)
            repository.quote(
                StayQuoteRequest(
                    checkInDate = state.checkInDate.toString(),
                    checkOutDate = state.checkOutDate.toString(),
                ...
                _uiState.value = _uiState.value.copy(
                    isLoading = false,
                    quote = quote,
                    couponInput = quote.couponCode.orEmpty(),
                    couponMessage = "Coupon ${quote.couponCode} applied",
                    couponError = null
                )
            }.onFailure { error ->
                _uiState.value = _uiState.value.copy(
                    isLoading = false,
                    couponError = error.message ?: "Unable to apply coupon",
                    couponMessage = null
                )
            }
        }
